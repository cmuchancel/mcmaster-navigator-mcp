...
The final exact-recovery run used 250 cases sampled across 25 catalog categories, later summarized in Fig.~\ref{fig:category-coverage}.
...
\centering
\begin{tikzpicture}[
  outcome/.style={draw, rounded corners=2pt, fill=black!3, minimum width=4.25cm, minimum height=2.2cm, align=center, inner sep=4pt}
]
\node[outcome] (exact) at (0,0) {
  {\bfseries\small Exact}\\[2pt]
  {\bfseries\Large 250/250}\\[-1pt]
  {\scriptsize one held-out part recovered}
};
\node[outcome] (multi) at (4.75,0) {
  {\bfseries\small Multi-match}\\[2pt]
  {\bfseries\Large 50/50}\\[-1pt]
  {\scriptsize alternatives preserved}
};
\node[outcome] (none) at (9.50,0) {
  {\bfseries\small No match}\\[2pt]
  {\bfseries\Large 25/25}\\[-1pt]
  {\scriptsize empty set returned}
};
\node[font=\scriptsize, align=center] at (4.75,-1.55) {All completed benchmark regimes returned the expected match-set behavior.};
\end{tikzpicture}
...
Figure~\ref{fig:benchmark-outcomes} summarizes the completed benchmark runs.

\subsection{Exact Part Recovery Results}

The exact-recovery benchmark returned exactly one part number for all 250 cases, and that part number was the held-out target in every case.
...
The multi-match benchmark passed all 50 under-specified cases. For controlled cases, the resolver returned the expected matching set. For broad category-level descriptions, it returned multiple part numbers rather than collapsing the request to a single guess. These results are reported together because they test the same contract: when a description admits more than one catalog row, the returned object should preserve the alternatives.
