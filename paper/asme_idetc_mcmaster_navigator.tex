...
A prior public repository, \tool{mjbraun/mcmaster-agent}, exposes an MCP server with three documented tools~\cite{mcmasteragent}. The first, \tool{mcmaster\_get\_product}, takes a known part number as input and returns a product name, product URL, and specification fields. The second, \tool{mcmaster\_search}, takes a keyword query such as ``ball valve brass'' and returns a list of search results with part numbers, names, and URLs. The third, \tool{mcmaster\_url}, takes either a part number or a search query and returns the corresponding McMaster-Carr URL. Its implementation uses SeleniumBase UC mode, cookie persistence, and a visible browser window because the repository documentation reports that McMaster-Carr blocks headless browsers.

Those tools cover lookup and search. Their input-output contract is either part number $\rightarrow$ product information, query $\rightarrow$ search results, or part/query $\rightarrow$ URL. The tool developed in this paper has a different contract: textual component description $\rightarrow$ all matching part numbers. For example, an engineering agent should be able to provide a detailed requirement for a screw, hinge, filter, bearing, or other catalog component and receive the McMaster-Carr part numbers whose catalog rows satisfy that requirement. The server supports this by navigating rendered pages, extracting the live table columns and row values, aligning the textual constraints to those live fields, and filtering rows programmatically. This is the differentiating step needed before downstream product-data, CAD, datasheet, and BOM retrieval can be called.
