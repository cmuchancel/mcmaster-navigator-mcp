...
\centering
\begin{tikzpicture}[
  outcome/.style={draw, rounded corners=2pt, fill=black!3, minimum width=3.75cm, minimum height=2.2cm, align=center, inner sep=4pt}
]
...
\node[outcome] (multi) at (4.05,0) {
  {\bfseries\small Known multi-match}\\[2pt]
  {\bfseries\Large 25/25}\\[-1pt]
  {\scriptsize expected part set recovered}
};
\node[outcome] (none) at (8.10,0) {
  {\bfseries\small No match}\\[2pt]
  {\bfseries\Large 25/25}\\[-1pt]
  {\scriptsize empty set returned}
};
\node[outcome] (broad) at (12.15,0) {
  {\bfseries\small Broad multi-match}\\[2pt]
  {\bfseries\Large 25/25}\\[-1pt]
  {\scriptsize multiple parts returned}
};
\node[font=\scriptsize, align=center] at (6.075,-1.55) {All completed benchmark regimes returned the expected match-set cardinality and contents.};
\end{tikzpicture}
\caption{Benchmark outcome summary. The resolver returns one part for sufficiently specific descriptions, all expected alternatives for known multi-match descriptions, no parts for impossible descriptions, and multiple parts for broad under-specified descriptions.}
...
The known multi-match benchmark returned the expected matching set in all 25 cases. The broad multi-match benchmark also returned multiple part numbers for all 25 under-specified category descriptions, showing that the resolver preserves alternatives instead of collapsing broad requests to a single guess.
